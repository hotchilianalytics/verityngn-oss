...
from verityngn.config.settings import AGENT_MODEL_NAME, GOOGLE_SEARCH_API_KEY, CSE_ID, MAX_OUTPUT_TOKENS_2_0_FLASH
...
def get_agent(state):
    ...
    llm = VertexAI(model_name=AGENT_MODEL_NAME,
                   temperature=0.7, max_output_tokens=MAX_OUTPUT_TOKENS_2_0_FLASH)
    
    prompt = ChatPromptTemplate.from_template("""You are an expert fact-checker with a focus on debunking misinformation. Your task is to verify claims by analyzing provided evidence and determining their veracity.
...
def search_youtube_counter_intel_standalone(video_url: str, video_id: str) -> List[Dict[str, Any]]:
    """
    ...
    logger.info(f"🎯 [YOUTUBE COUNTER-INTEL] Starting independent search for {video_id}")
        ...
        for dir_path in possible_dirs:
            ...
            if os.path.exists(info_file):
                with open(info_file, 'r') as f:
                    ...
                    logger.info(f"Found video title from existing info: {video_title[:50]}...")
                    break
...
def collect_and_group_evidence(
    ...
    press_release_domains = [
        ...
        'prlog.org', 'openpr.com', 'pressat.co.uk', 'pressreleases.com',
        'pr.com', 'pr-inside.com', 'prurgent.com', 'prfire.co.uk', 'prbuzz.com',
        'prnews.biz', 'pressbox.co.uk', 'pressreleasepoint.com',
        '24-7pressrelease.com', 'abnewswire.com', 'webwire.com', 'sbwire.com',
        'releasewire.com', 'prsync.com', 'przoom.com', 'prfree.org', 'prdistribution.com'
    ]
    
    # Scientific domain patterns
    scientific_domains = [
        'pubmed.ncbi.nlm.nih.gov', 'nature.com', 'science.org', 'arxiv.org',
        'nih.gov', 'who.int', 'fda.gov', 'cdc.gov', 'nejm.org', 'bmj.com',
        'lancet.com', 'cell.com', 'pnas.org', 'jama.jamanetwork.com'
    ]
    ...
    for evidence in all_evidence:
        ...
        is_press_release = (
            source_type == 'press release' or
            any(domain in url for domain in press_release_domains)
        )
        
        is_youtube_counter = source_type == 'youtube_counter_intelligence'
        
        is_scientific = (
            source_type in ('scientific journal', 'academic research', 'peer-reviewed') or
            any(domain in url for domain in scientific_domains) or
            url.endswith('.edu')
        )
        ...
        if is_press_release:
            ...
            if video_channel and video_channel.lower() in content_to_check:
                ...
                logger.info(f"⚠️ Likely self-referential press release (term matches): {url[:50]}...")
        ...
        if is_scientific and text:
            support_indicators = ['effective', 'significant', 'proven', 'benefit', 'successful', 'improves']
            oppose_indicators = ['ineffective', 'no effect', 'placebo', 'not significant', 'failed']
    ...
    logger.info(f"📊 Evidence groups: Independent={len(evidence_groups['independent'])}, "
    ...
    self_ref_count = sum(1 for e in evidence_groups['press_releases'] if e.get('self_referential', False))
    if self_ref_count > 0:
        logger.warning(f"🚫 Found {self_ref_count} self-referential press releases that cannot validate claims")
    
    return evidence_groups

def generate_press_release_counter_boost(press_release_evidence: List[Dict[str, Any]], claim_text: str) -> Dict[str, Any]:
    """
    ...
    self_referential_count = sum(1 for e in press_release_evidence if e.get('self_referential', False))
    ...
    if self_referential_count > 0:
        boost_info["probability_adjustment"] = -0.25  # Stronger penalty for self-referential
        boost_info["explanation"] = f"Found {self_referential_count}/{total_pr_count} self-referential press releases promoting the same product/service being claimed. This creates circular validation where promotional content supports its own claims."
        ...
        boost_info["explanation"] = f"Multiple press releases ({total_pr_count}) found supporting this claim, but press releases are promotional content with inherent bias toward positive claims about products/services."
        ...
        boost_info["explanation"] = f"Press release evidence found, but press releases are promotional materials with inherent bias toward supporting product claims rather than independent verification."
    ...
    boost_info["analysis"] = f"Counter-intelligence analysis: Press releases reduced claim credibility by {abs(boost_info['probability_adjustment'])*100:.0f}% due to promotional bias and lack of independent validation."
    
    return boost_info

def generate_youtube_counter_boost(youtube_evidence: List[Dict[str, Any]], claim_text: str) -> Dict[str, Any]:
    """
    ...
    for video in youtube_evidence:
        ...
        negative_keywords = ['scam', 'fake', 'fraud', 'doesn\'t work', 'waste of money', 'warning', 'beware', 'exposed', 'debunk']
    ...
    if len(high_credibility_videos) > 0:
        base_adjustment += len(high_credibility_videos) * 0.05  # Additional weight for credible sources
    
    # Factor 3: Multiple negative videos (but with diminishing returns)
    if len(negative_videos) >= 2:
        negative_boost = min(0.08, len(negative_videos) * 0.03)  # Max 8% from multiple negatives
    ...
    if len(negative_videos) >= 2:
        if total_views > 10000:
            boost_info["explanation"] = f"Multiple YouTube videos ({len(negative_videos)}/{len(youtube_evidence)}) contain negative reviews or warnings about this claim. Independent reviewers with {total_views:,} total views provide contradictory evidence with significant reach."
        else:
            boost_info["explanation"] = f"Multiple YouTube videos ({len(negative_videos)}/{len(youtube_evidence)}) contain negative reviews or warnings about this claim. Independent reviewers with {total_views:,} total views provide contradictory evidence with limited reach."
        ...
        boost_info["explanation"] = f"High-credibility YouTube reviews ({len(high_credibility_videos)} videos with {total_views:,} total views) provide counter-intelligence to this claim through independent analysis."
        ...
        boost_info["explanation"] = f"YouTube counter-intelligence found {len(youtube_evidence)} videos providing alternative perspectives on this claim, indicating public scrutiny and contradictory viewpoints."
    ...
    boost_info["analysis"] = f"Counter-intelligence analysis: YouTube reviews reduced claim credibility by {abs(boost_info['probability_adjustment'])*100:.0f}% due to independent contradictory evidence from {len(youtube_evidence)} sources with {total_views:,} total views."
...
def apply_counter_intelligence_boosts(
    ...
    if press_release_evidence:
        pr_boost = generate_press_release_counter_boost(press_release_evidence, claim_text)
        if pr_boost:
            ...
            enhanced_dist["TRUE"] = max(0.0, enhanced_dist["TRUE"] - true_reduction * 0.6)  # Only apply 60% of reduction
            enhanced_dist["FALSE"] = min(1.0, enhanced_dist["FALSE"] + true_reduction * 0.4)  # 40% goes to FALSE (reduced from 70%)
            enhanced_dist["UNCERTAIN"] = min(1.0, enhanced_dist["UNCERTAIN"] + true_reduction * 0.4)  # 40% to UNCERTAIN (increased from 30%)
    ...
    if youtube_evidence:
        ...
        if youtube_boost:
            ...
            # Reduce TRUE probability, increase FALSE probability (ENHANCED - Research Path)
            enhanced_dist["TRUE"] = max(0.0, enhanced_dist["TRUE"] - true_reduction * 0.5)  # Only apply 50% of reduction
            enhanced_dist["FALSE"] = min(1.0, enhanced_dist["FALSE"] + true_reduction * 0.4)  # 40% goes to FALSE (reduced from 80%)
            enhanced_dist["UNCERTAIN"] = min(1.0, enhanced_dist["UNCERTAIN"] + true_reduction * 0.5)  # 50% to UNCERTAIN (increased from 20%)
    
    # Apply evidence quality boost to counteract over-conservative estimates
    enhanced_dist = apply_evidence_quality_boost(enhanced_dist, youtube_evidence, press_release_evidence)
...
def apply_evidence_quality_boost(prob_dist: Dict[str, float], youtube_evidence: List[Dict[str, Any]], press_release_evidence: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    youtube_total_views = sum(video.get('view_count', 0) for video in youtube_evidence) if youtube_evidence else 0
    ...
    if youtube_total_views < 1000 and press_total_reach < 3:
        ...
        if boosted_dist['TRUE'] > boosted_dist['FALSE']:
            ...
            boosted_dist['UNCERTAIN'] = max(0.05, boosted_dist['UNCERTAIN'] - confidence_boost * 0.7)
            boosted_dist['FALSE'] = max(0.05, boosted_dist['FALSE'] - confidence_boost * 0.3)
        elif boosted_dist['FALSE'] > boosted_dist['TRUE']:
            boosted_dist['FALSE'] = min(0.95, boosted_dist['FALSE'] + confidence_boost)
            boosted_dist['UNCERTAIN'] = max(0.05, boosted_dist['UNCERTAIN'] - confidence_boost * 0.7)
            boosted_dist['TRUE'] = max(0.05, boosted_dist['TRUE'] - confidence_boost * 0.3)
    
    return boosted_dist

def verify_claim(state: ClaimVerificationState) -> Dict[str, Any]:
    """Verify a claim using evidence and search results."""
    from verityngn.services.search.youtube_search import search_youtube_counter_intelligence
    ...
    if not state.evidence:
        try:
            ...
            logger.info(f"Gathered {len(state.evidence)} pieces of evidence for claim: {claim_text[:100]}...")
        except Exception as e:
    ...
    press_release_associated = any(e.get('self_referential', False) for e in press_release_evidence)
    youtube_negative_found = len(youtube_review_evidence) > 0
    scientific_evidence_found = len(scientific_evidence) > 0
    scientific_supports = any(e.get('supports_claim', False) for e in scientific_evidence)
    ...
    if main_evidence:
        evidence_text = "\n".join([
            ...
            for item in main_evidence[:10]  # Limit evidence
        ])
    ...
    try:
        ...
        prob_dist = result.get("probability_distribution", {"TRUE": 0.15, "FALSE": 0.50, "UNCERTAIN": 0.35})
        ...
        def calculate_enhanced_probability_distribution(
            ...
            modifications.append(f"Validation power totals: Independent={independent_power:.1f}, Press={press_release_power:.1f}, Scientific={scientific_power:.1f}, YouTube={youtube_counter_power:.1f}")
            
            # Factor 1: Evidence quality based on validation power
            if total_validation_power > 0:
                quality_factor = min(1.0, total_validation_power / 5)  # Scale based on effective evidence
                uncertainty_reduction = enhanced_dist.get("UNCERTAIN", 0.3) * quality_factor * 0.2
                enhanced_dist["UNCERTAIN"] = max(0.05, enhanced_dist.get("UNCERTAIN", 0.3) - uncertainty_reduction)
                modifications.append(f"Evidence quality (power={total_validation_power:.1f}) reduces uncertainty by {uncertainty_reduction:.2f}")
            ...
            if total_validation_power > 0:
                ...
                if promotional_power_ratio > 0.6:  # Majority promotional power
                    ...
                    enhanced_dist["FALSE"] = min(0.9, enhanced_dist.get("FALSE", 0.3) + false_boost)
                    modifications.append(f"High promotional power ratio ({promotional_power_ratio:.1%}) increases FALSE by {false_boost:.2f}")
                elif independent_power_ratio > 0.7:  # Majority independent power
                    true_boost = min(0.3, independent_power_ratio * 0.3)
                    enhanced_dist["TRUE"] = min(0.85, enhanced_dist.get("TRUE", 0.3) + true_boost)
                    modifications.append(f"High independent power ratio ({independent_power_ratio:.1%}) increases TRUE by {true_boost:.2f}")
            
            # Factor 3: Scientific evidence weighting
            if scientific_power > 0:
                science_weight = min(0.4, scientific_power * 0.2)
                enhanced_dist["TRUE"] = min(0.85, enhanced_dist.get("TRUE", 0.3) + science_weight)
                modifications.append(f"Scientific evidence (power={scientific_power:.1f}) increases TRUE by {science_weight:.2f}")
            
            # Factor 4: YouTube counter-intelligence impact (ENHANCED - Research Path)
            if youtube_counter_power > 0:
                youtube_impact = min(0.20, youtube_counter_power * 0.08)  # Reduced from 0.35 and 0.15
                enhanced_dist["FALSE"] = min(0.85, enhanced_dist.get("FALSE", 0.3) + youtube_impact)  # Reduced max from 0.9
                modifications.append(f"YouTube counter-intelligence (power={youtube_counter_power:.1f}) increases FALSE by {youtube_impact:.2f}")
            
            # Factor 5: Self-referential press release penalty
            self_ref_count = sum(1 for e in evidence_groups["press_releases"] if e.get('self_referential', False))
            if self_ref_count > 0:
                penalty = min(0.4, self_ref_count * 0.15)
                enhanced_dist["FALSE"] = min(0.9, enhanced_dist.get("FALSE", 0.3) + penalty)
                modifications.append(f"Self-referential press releases ({self_ref_count}) penalty increases FALSE by {penalty:.2f}")
            
            # Normalize to ensure probabilities sum to 1.0
            total = sum(enhanced_dist.values())
            if total > 0:
                enhanced_dist = {k: round(v / total, 3) for k, v in enhanced_dist.items()}
        ...
        enhanced_prob_dist, prob_modifications = calculate_enhanced_probability_distribution(
            prob_dist,
            evidence_groups
        )
        ...
        if press_release_found:
            self_ref_count = sum(1 for e in press_release_evidence if e.get('self_referential', False))
            total_pr_power = sum(e.get('validation_power', 1.0) for e in press_release_evidence)
            
            # Add press release counter-intelligence explanation with quotes
            pr_boosts = [b for b in counter_intel_boosts if b.get('type') == 'press_release_counter']
            if pr_boosts:
                pr_boost = pr_boosts[0]
                explanation_add.append(f"PRESS RELEASE COUNTER-INTELLIGENCE: {pr_boost['explanation']}")
                
                # Add quotes if available
                for quote in pr_boost.get('quotes', [])[:2]:  # Limit to 2 quotes
                    explanation_add.append(f"   📋 {quote['source']}: {quote['quote']} → {quote['analysis']}")
            
            if self_ref_count > 0:
                explanation_add.append(f"🚫 SELF-REFERENTIAL EVIDENCE: {self_ref_count} press releases detected that reference the same product/video being analyzed. These sources have zero validation power for their own claims.")
            
            if total_pr_power < len(press_release_evidence) * 0.5:
                explanation_add.append(f"LIMITED PRESS RELEASE VALIDATION: Press release sources have reduced validation power (total={total_pr_power:.1f}/{len(press_release_evidence)}) due to promotional nature and self-referential content.")
        
        if youtube_negative_found:
            youtube_power = sum(e.get('validation_power', 1.0) for e in youtube_review_evidence)
            
            # Add YouTube counter-intelligence explanation with quotes
            youtube_boosts = [b for b in counter_intel_boosts if b.get('type') == 'youtube_counter']
            if youtube_boosts:
                youtube_boost = youtube_boosts[0]
                explanation_add.append(f"📺 YOUTUBE COUNTER-INTELLIGENCE: {youtube_boost['explanation']}")
                
                # Add quotes if available
                for quote in youtube_boost.get('quotes', [])[:2]:  # Limit to 2 quotes
                    explanation_add.append(f"   {quote['source']}: {quote['quote']} → {quote['analysis']}")
            else:
                explanation_add.append(f"YOUTUBE COUNTER-INTELLIGENCE: Negative/contradictory YouTube evidence found (validation power={youtube_power:.1f}). These reviews provide independent contradictory perspective.")
            
        if scientific_evidence_found:
            scientific_power = sum(e.get('validation_power', 1.0) for e in scientific_evidence)
            supporting_scientific = sum(1 for e in scientific_evidence if e.get('supports_claim', False))
            explanation_add.append(f"SCIENTIFIC EVIDENCE: {len(scientific_evidence)} scientific sources (power={scientific_power:.1f}), {supporting_scientific} supporting claim.")
        
        if independent_evidence_found:
            independent_power = sum(e.get('validation_power', 1.0) for e in main_evidence)
            explanation_add.append(f"INDEPENDENT EVIDENCE: {len(main_evidence)} independent sources (validation power={independent_power:.1f}) provide unbiased perspective.")
        ...
        verification_result = {
            ...
            "sources": list(set(result.get("sources", []) + [item.get("url", "") for item in main_evidence if item.get("url")])),
        ...
        if state.evidence:
            ...
            evidence_summary += "; ".join([e.get('text', '')[:200] for e in state.evidence[:3] if e.get('text')])
            return {
                "result": "LIKELY_FALSE",
                "explanation": f"Error during verification, and available evidence does not strongly support the claim. Error: {str(e)}",
                "evidence": evidence_summary,
                "probability_distribution": {"TRUE": 0.05, "FALSE": 0.85, "UNCERTAIN": 0.10},
                "sources": [item.get("url", "") for item in state.evidence if item.get("url")],
                "pr_sources": [],
                "youtube_counter_sources": []
            }
        return {
...
def verify_claim_node(state: ClaimVerificationState) -> ClaimVerificationState:
    ...
    try:
        ...
        logger.info(f"Verification completed with result: {verification_result['result']}")
...
def process_claims(state: InitialAnalysisState) -> InitialAnalysisState:
    ...
    for idx, claim in enumerate(state.claims):
        logger.info(f"Processing claim {idx + 1}/{len(state.claims)}: {get_value(claim, 'claim_text', '')[:100]}...")
        
        # Create verification state
        verification_state = ClaimVerificationState(
            claim=claim,
            video_url=state.video_url,
            video_title=get_value(state.media_embed, "title", "") if state.media_embed else "",
            evidence=[]
        )
        ...
        try:
            ...
            if isinstance(claim, dict):
                ...
                updated_claim = {k: getattr(claim, k) for k in dir(claim) 
            ...
            if verification_state.verification_result:
                updated_claim["verification_result"] = verification_state.verification_result
                updated_claim["explanation"] = get_value(verification_state.verification_result, "explanation", "")
            else:
                ...
                updated_claim["explanation"] = "Verification process failed to produce a result"
            ...
            if verification_state.evidence:
                ...
                evidence_entry = {
                    ...
                    "source_url": "",
                    "text": verification_state.verification_result.get("evidence", "No evidence summary available"),
                ...
                for evidence in verification_state.evidence:
                    if isinstance(evidence, dict) and evidence.get('text'):
                        source_entry = {
                            "source_name": evidence.get("source_name", "Referenced Source"),
                            "source_type": evidence.get("source_type", "web"),
                            "title": evidence.get("title", evidence.get("source_url", "")),
                            "source_url": evidence.get("source_url", ""),
                            "text": evidence.get("text", ""),
                            "credibility_level": evidence.get("credibility_level", "MEDIUM"),
                            "justification": evidence.get("justification", "External source referenced during verification")
                        }
    ...
    logger.info(f"Processed {len(updated_claims)} claims with {len(aggregated_evidence)} pieces of evidence")
    
    # Update state with verified claims and evidence (preserve existing keys on dict state)
    if isinstance(state, dict):
        return {**state, "claims": updated_claims, "aggregated_evidence": aggregated_evidence}
    return state.model_copy(update={"claims": updated_claims, "aggregated_evidence": aggregated_evidence})
...
async def run_claim_verification(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    initial_review_text = init_rpt.get("initial_report","") + init_rpt.get("video_analysis_summary", "")
    ...
    logger.info(f"🎯 [COUNTER-INTEL] Processed {len(youtube_counter_evidence)} counter-intelligence sources from CI-once")
    ...
    try:
        ...
        if video_duration_minutes == 30.0:  # Default fallback, try to get actual duration
            try:
                ...
                duration_seconds = (
                    video_info.get('duration') or 
                    media_embed.get('duration') or
                    state.get('video_duration_seconds') or
                    1800  # 30 minutes fallback
                )
                video_duration_minutes = duration_seconds / 60.0
                logger.info(f"📊 Calculated video duration: {video_duration_minutes:.1f} minutes from {duration_seconds} seconds")
            except Exception as e:
                logger.warning(f"Could not calculate video duration: {e}, using 30 minute default")
                video_duration_minutes = 30.0
        
        output_dir = f"sherlock_analysis_{video_id}"  # Use existing analysis directory
        
        logger.info(f"🚀 [CLAIMS] Starting advanced claim processing for {len(claims)} claims")
        ...
        logger.info(f"✅ Advanced claim processing selected {len(claims_to_process)} claims from {len(claims)} total video analysis claims")
        
        # Process each claim using the enhanced verification system
        for i, claim in enumerate(claims_to_process):
            claim_text = claim.get('claim_text', '')
            logger.info(f"🔎 Verifying claim {i+1}/{len(claims_to_process)}: {claim_text[:100]}...")
            
            try:
                # Create object wrapper for enhanced system compatibility
                class StateWrapper:
                    def __init__(self, claim, video_url, video_title, video_id, media_embed):
                        self.claim = claim
                ...
                logger.info(f"🔍 [CLAIM {i+1}] Starting verification for: {claim_text[:80]}...")
                
                # Use the enhanced verify_claim function
                verification_result = verify_claim(verification_state)
                
                logger.info(f"✅ [CLAIM {i+1}] Verification completed, evidence count: {len(verification_result.get('evidence', []))}")
                
                # Add rate limiting between claims to avoid API throttling
                import asyncio
                if i < len(claims_to_process) - 1:  # Don't sleep after the last claim
                    await asyncio.sleep(2)  # 2 second delay between claims
                    ...
                    for item in raw_evidence:
                        if isinstance(item, EvidenceSource):
                            ...
                            try:
                                ...
                                logger.warning(f"Failed to convert dict to EvidenceSource: {e}")
                        elif isinstance(item, str):
                            evidence_list.append(EvidenceSource(
                                source_name=f"Source: {item.split('/')[-1][:50] if item.startswith('http') else 'Evidence'}",
                                source_type="url" if item.startswith('http') else "text",
                ...
                if hasattr(verification_result, 'pr_sources') and verification_result.get('pr_sources'):
                    pr_count = len(verification_result['pr_sources'])
                    logger.info(f"📰 Claim {i+1}: Found {pr_count} press release sources")
                
                if hasattr(verification_result, 'youtube_counter_sources') and verification_result.get('youtube_counter_sources'):
                    youtube_count = len(verification_result['youtube_counter_sources'])
                    logger.info(f"📺 Claim {i+1}: Found {youtube_count} YouTube counter-intelligence sources")
                
            except Exception as e:
                logger.error(f"❌ Failed to verify claim {i+1} with enhanced system: {e}")
        ...
        logger.info(f"✅ Completed ENHANCED verification: {len(verified_claims)} claims, {len(all_evidence)} evidence items")
...
async def search_youtube_reviews(claim_text: str, video_title: str, max_results: int = 3) -> list:
...
async def search_claim_evidence(search_tool: SearchTool, claim: Dict[str, Any], video_url: str, video_title: str = "") -> List[Dict[str, Any]]:
...
async def verify_claim_with_evidence(claim: Dict[str, Any], evidence: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        ...
        llm = ChatVertexAI(model_name=AGENT_MODEL_NAME, temperature=0.1)
        ...
        try:
            ...
            if "probability_distribution" not in result:
                result["probability_distribution"] = {"TRUE": 0.1, "FALSE": 0.1, "UNCERTAIN": 0.8}
            
            # Add source URLs
            result["sources"] = [item.get("url", "") for item in evidence if item.get("url")]
            return result
            ...
            if evidence and len(evidence) > 0:
                return {
                    ...
                    "probability_distribution": {"TRUE": 0.3, "FALSE": 0.3, "UNCERTAIN": 0.4},
                ...
                return {
                    ...
                    "probability_distribution": {"TRUE": 0.05, "FALSE": 0.85, "UNCERTAIN": 0.1},
                    "sources": []
                }
...
def process_claims_with_advanced_ranking(
    ...
    logger.info(f"✅ Advanced claim processing complete: {len(final_claims)} claims selected for verification")
